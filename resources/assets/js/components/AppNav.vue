    ...
    <div class="container d-flex align-items-center justify-content-around px-2 px-md-3 nav-padding">
        ...
        <transition name="logo">
            <div class="d-flex align-items-baseline">
                ...
                <a href="/commit" class="ml-2 text-main" target="_blank">Beta 1.0.5</a>
            </div>
        </transition>


        <!--搜索-->
        <div class="position-relative" :style="{width:searchWidth}">
            <!--搜索框-->
            <form @submit="onSubmit" action="/search" class="form-inline box-shadow" target="_blank">
                <div class="input-group w-100">
                    <input v-model.trim="query" @focus="onfocus" @blur="onblur"
                           type="search" name="search" class="form-control" placeholder="看看这个化妆品好不好用"
                           autocomplete="off" maxlength="64">
                    <div class="input-group-append">
                        <button :disabled="query.length===0" class="btn btn-secondary rounded-right" type="submit">
            ...
            <!--搜索下拉结果框-->
            <div class="list-group position-absolute w-100 bg-white border rounded box-shadow-lg mt-1"
                 v-show="searchFrame">
                    ...
                    <div class="list-group-item" v-if="searchHint" key="ing">
                        <transition name="fade" mode="out-in">
                            <div v-if="searching" key="searching">
                                <img class="mx-auto loading-size" :src="`${storageUrl}/icons/loading.gif`"
                                     alt="正在玩命找...">
                    ...
                    <div v-else key="result">
                               ...
                               class="list-group-item list-group-item-action px-2 px-md-4">
                                <div class="media align-items-center">
                                    ...
                                    <div class="media-body">
                                        <h5 class="text-brown mb-2">{{cat.name}}</h5>
                        ...
                        <template v-if="!!brand">
                            <a :href="`/brands/${brand.id}`" @mousedown.prevent target="_blank"
                               class="list-group-item list-group-item-action px-2 px-md-4">
                                <div class="media align-items-center">
                                    <img class="mr-md-3 product-s-size"
                                         :src="`${storageUrl}/brands/${brand.id}.jpg!brand`" :alt="brand.name">
                                    <div class="media-body">
                        ...
                        <template v-if="products.length>0">
                            <a :href="`/products/${product.id}`" v-for="product in products" target="_blank"
                               :key="`product${product.id}`" @mousedown.prevent
                               class="list-group-item list-group-item-action px-2 px-md-4">
                                <div class="media align-items-center">
                                    <img class="mr-md-3 product-s-size"
                                         :src="`${storageUrl}/products/${product.id}.jpg!cosme`" :alt="product.name">
                                    <div class="media-body">
                                        ...
                                        <div class="d-flex align-items-end my-1">
                                            <ProductRate class="mr-2 mr-md-3 text-normal" :rate="product.rate"/>
                        ...
                        <a href="https://www.algolia.com/" @mousedown.prevent target="_blank"
                           class="list-group-item list-group-item-action d-flex justify-content-end">
                            <svg width="130" height="18" viewBox="0 0 130 18" xmlns="http://www.w3.org/2000/svg">
                                <defs>
...
<script>
    ...
    import LoginFormMobile from './user/LoginFormMobile'
    ...
    export default {
        ...
        methods: {
            onfocus() {
                ...
                if (this.query.length > 0) this.searchFrame = true;
            },
            onblur() {
                if (this.isMobile) {
                    setTimeout(() => {
                ...
                if (this.query.length > 0) this.searchFrame = false;
            },
            search: _.debounce(
                function () {
