...
@section('content')
    ...
    <div class="{{$is_tablet ? 'offset-md-1 col-md-10' : 'col-md-7'}}">
        <div class="media align-items-center mb-3 mb-md-0">
            ...
            @if($is_phone)
                <img src="{{config('app.url')}}/products/{{$product->id}}.jpg!product" class="product-size"
                     alt="{{$product->name}}">
            @else
                <img class="mr-3 product-l-size" src="{{config('app.url')}}/products/{{$product->id}}.jpg!product.l"
                     alt="{{$product->name}}">
            @endif
            {{--商品的右边部分--}}
